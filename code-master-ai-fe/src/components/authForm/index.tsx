...
export default function AuthForm({ type = "login" }: AuthFormProps) {
  ...
  const onConfirmPasswordChange = (value: string) => {
    setErrorConfirmPassword(value !== formRegisterData.password);
    setFormRegisterData({ ...formRegisterData, confirmPassword: value });
  };
  ...
  const onSubmit = async () => {
    ...
    if (tab === "login") {
      try {
        ...
        // ✅ Lỗi trùng phương thức → hiển thị UI đẹp
        if (error.response?.status === 400 && messageFromServer.includes("phương thức")) {
          setConflictError(messageFromServer);
        } else {
          // Sai email/password → toast bình thường
          showMessage("error", messageFromServer || "Đăng nhập thất bại");
        }
      }
    } else {
      try {
    ...
    <div className="w-full max-w-[390px] py-2">
      ...
      <Modal
        title={
          ...
          : forgotStep === "otp" ? "Nhập mã xác nhận"
          : "Đặt lại mật khẩu"
        }
        open={isForgotModalOpen}
        onCancel={() => setIsForgotModalOpen(false)}
      ...
      >
        <div className="pt-2">
          {forgotStep === "email" && (
            <>
              <p className="text-sm text-gray-500 mb-3">
                Vui lòng nhập email đã đăng ký, chúng tôi sẽ gửi mã xác nhận (OTP) gồm 5 chữ số cho bạn.
              </p>
              <Input
                placeholder="Nhập email của bạn..."
                value={forgotEmail}
